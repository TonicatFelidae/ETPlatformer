...
{
    public class Sensor_ray2D : BaseSensor
    {
        ...
        public void FixedUpdate()
        {
            ...
            switch (rayType)
            {
                case SensorRay2DType.Single:
                    _rayHit[0] = data.detectedRaycastHit2D[0] = Physics2D.Raycast(transform.position, transform.TransformDirection(Vector2.right), _distance);
                    break;
                case SensorRay2DType.All:
                    _rayHit = data.detectedRaycastHit2D = Physics2D.RaycastAll(transform.position, transform.TransformDirection(Vector2.right), _distance);
                    break;
                default:
                    break;
            }
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            Vector3 direction = transform.TransformDirection(Vector3.right * gizmo_distance);
